...
class SecuritySystem {
    ...
    private function createSecurityTables() {
        ...
        // Vérifier d'abord que la table utilisateurs existe et est accessible
        try {
            // Vérifier que la table existe
            $stmt = $this->pdo->query("SHOW TABLES LIKE 'utilisateurs'");
            if ($stmt->rowCount() == 0) {
                // La table utilisateurs n'existe pas encore, créer les tables sans clés étrangères
                $this->createSecurityTablesWithoutFK();
                return;
            }
            
            // Vérifier que la table est accessible et a la bonne structure
            $stmt = $this->pdo->query("DESCRIBE utilisateurs");
            $columns = $stmt->fetchAll(PDO::FETCH_COLUMN);
            if (!in_array('id', $columns)) {
                // La table n'a pas la colonne id, créer les tables sans clés étrangères
                $this->createSecurityTablesWithoutFK();
                return;
            }
            
            // Vérification supplémentaire : essayer de faire une requête simple
            $stmt = $this->pdo->query("SELECT COUNT(*) FROM utilisateurs LIMIT 1");
            if (!$stmt) {
                // La table n'est pas accessible, créer les tables sans clés étrangères
                $this->createSecurityTablesWithoutFK();
                return;
            }
            
        } catch (Exception $e) {
            // En cas d'erreur, créer les tables sans clés étrangères
            $this->createSecurityTablesWithoutFK();
            return;
        }
    ...
    /**
     * Créer les tables de sécurité sans clés étrangères (quand utilisateurs n'existe pas encore)
     */
    private function createSecurityTablesWithoutFK() {
        // Table des tentatives de connexion
        $sql = "CREATE TABLE IF NOT EXISTS login_attempts (
            id INT AUTO_INCREMENT PRIMARY KEY,
            ip_address VARCHAR(45) NOT NULL,
